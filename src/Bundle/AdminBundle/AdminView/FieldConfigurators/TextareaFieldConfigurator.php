...
class TextareaFieldConfigurator extends GenericFieldConfigurator
{
    /**
     * {@inheritDoc}
     */
    ...
    {
        return file_get_contents(__DIR__ . '/../../Resources/GraphQL/Schema/AdminViewField/textarea.graphql');
    }
    ...
    protected function processWysiwygAdminFieldDirective(array $directive, AdminViewField $field) {
        ...
        if($field->getFieldType() !== TextType::getType()) {
            throw new InvalidAdminViewFieldConfig(sprintf(
                'You can only use the @textareaAdminField directive on fields of type %s, however "%s" is a "%s".',
                TextType::getType(),
                $field->getId(),
                $field->getFieldType()
            ));
    ...
    public function configureField(AdminViewField $field, AdminView $adminView, ContentType $contentType) {
        foreach($field->getDirectives() as $directive) {
            if($directive['name'] === 'textareaAdminField') {
