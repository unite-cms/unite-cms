<template>
  <section class="uk-section uk-position-relative">
    <div class="uk-container">
      ...
      <component :is="$listeners.submit ? 'form' : 'div'" :class="card ? 'uk-card uk-card-default uk-negative-padding-small@s' : null" :method="$listeners.submit ? 'POST' : null" :enctype="$listeners.submit ? 'multipart/form-data' : null" @submit.prevent="submit">

        <div :class="card ? 'uk-card-header' : 'detail-header'" v-if="$slots.header">
          <slot name="header"></slot>
        </div>

        <div :class="card ? 'uk-card-body uk-padding-small@s' : 'detail-body'">
          <slot></slot>
        </div>
