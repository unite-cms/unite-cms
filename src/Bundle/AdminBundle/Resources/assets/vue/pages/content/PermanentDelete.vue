<template>
  <content-detail :loading="loading || $apollo.loading" @submit="submit">
    <h1 class="uk-card-title">{{ $t('content.permanent_delete.headline', view) }}</h1>
    <p>{{ $t('content.permanent_delete.message') }}</p>
    <button slot="footer" class="uk-button uk-button-danger" type="submit">{{ $t('content.permanent_delete.actions.submit') }}</button>
  </content-detail>
</template>
