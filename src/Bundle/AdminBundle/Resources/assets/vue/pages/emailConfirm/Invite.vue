  ...
  <div class="uk-background-secondary uk-height-viewport uk-flex uk-flex-center uk-flex-middle">
    <div class="uk-card uk-card-default uk-padding" style="max-width: 600px;">
      <div v-if="tokenExpired">
        <alerts-list :alerts="{ message: $t('email_confirm.invite.token_expired'), type: 'danger' }" />
      </div>
      <form class="uk-form" v-else>

        <h1>{{ $t('email_confirm.invite.headline', { type }) }}</h1>
        <p>{{ $t('email_confirm.invite.text', { type }) }}</p>
        <alerts-list />

        <template v-if="!success">
          <div class="uk-margin">
            <label>{{ $t('email_confirm.invite.labels.username') }}</label>
            <input disabled type="text" class="uk-input" :value="username" />
          </div>
          <div class="uk-margin">
            <label>{{ $t('email_confirm.invite.labels.password') }}</label>
            <input type="password" class="uk-input" v-model="password" />
          </div>
          <div class="uk-margin">
            <label>{{ $t('email_confirm.invite.labels.password_repeat') }}</label>
            <input type="password" class="uk-input" v-model="password_repeat" />
          </div>
          <div class="uk-margin">
            <button class="uk-button uk-button-primary" :disabled="loading || !valid" @click.prevent="signup"><label>{{ $t('email_confirm.invite.actions.submit') }}</label></button>
          </div>
            ...
            signup() {
                ...
                this.$apollo.mutate({
                    mutation: gql`mutation($token: String!, $password: String!, $type: String!, $username: String!) {
                        unite {
                            emailInviteConfirm(token: $token, password: $password, type: $type, username: $username)
                        }
                    }`,
                    variables: {
                    ...
                    if(data.data.unite.emailInviteConfirm) {
                        Alerts.$emit('push', 'success', this.$t('email_confirm.invite.success'));
                        this.success = true;
                        User.$emit('login',
                            {
                                type: this.type,
                        ...
                        Alerts.$emit('push', 'danger', this.$t('email_confirm.invite.error'));
