...
class StorageFileDeleteUnused extends Command
{
    ...
    private function findNestedFileDefinitions(FieldableField $field, &$buckets)
    {
        ...
        if ($field->getType() == FileFieldType::TYPE || $field->getType() == ImageFieldType::TYPE) {
            ...
            if ($field->getEntity()->getRootEntity() instanceof ContentType) {
                foreach ($this->em->getRepository('UniteCMSCoreBundle:Content')->findBy(
                    ['contentType' => $field->getEntity()->getRootEntity()]
                ) as $content) {
                    $buckets[$bucket_path]['files'] = array_merge(
                        $buckets[$bucket_path]['files'],
                        $this->findNestedFieldData($content->getData(), $fieldPathParts)
                    );
                }
            }
            ...
            if ($field->getEntity()->getRootEntity() instanceof SettingType) {
                foreach ($this->em->getRepository('UniteCMSCoreBundle:Setting')->findBy(
                    ['settingType' => $field->getEntity()->getRootEntity()]
                ) as $setting) {
                    $buckets[$bucket_path]['files'] = array_merge(
                        $buckets[$bucket_path]['files'],
                        $this->findNestedFieldData($setting->getData(), $fieldPathParts)
                    );
                }
            }
        } else {
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->em->getRepository('UniteCMSCoreBundle:ContentTypeField')->findBy(
            ['type' => [FileFieldType::TYPE, ImageFieldType::TYPE, 'collection']]
        ) as $field) {
            $this->findNestedFileDefinitions($field, $buckets);
        }

        // Find all setting file fields.
        foreach ($this->em->getRepository('UniteCMSCoreBundle:SettingTypeField')->findBy(
            ['type' => [FileFieldType::TYPE, ImageFieldType::TYPE, 'collection']]
        ) as $field) {
            $this->findNestedFileDefinitions($field, $buckets);
        }
            ...
            if ($input->getOption('force')) {
                ...
                $output->writeln(
                    [
                        ...
                        '<bg=black;fg=white;options=bold>         '.count(
                            $objects_to_delete
                        ).' files where deleted.         </>',
                        '<bg=black;fg=white;options=bold>*****************************************</>',
                    ]
