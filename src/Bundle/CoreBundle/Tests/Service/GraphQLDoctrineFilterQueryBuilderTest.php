...
class GraphQLDoctrineFilterQueryBuilderTest extends TestCase
{
    ...
    public function testBuildingSimpleORandANDFilter()
    {
        ...
        $this->assertEquals(
            ['graphql_filter_builder_parameter1' => 123, 'graphql_filter_builder_parameter2' => '%value%'],
            $builder->getParameters()
        );
        $filter = $builder->getFilter();
        $this->assertInstanceOf(Orx::class, $filter);
        $this->assertEquals(
            "c.id = :graphql_filter_builder_parameter1 OR JSON_EXTRACT(c.data, '$.any_field') LIKE :graphql_filter_builder_parameter2",
            (string)$filter
        );
        ...
        $this->assertEquals(
            "c.id = :graphql_filter_builder_parameter1 AND JSON_EXTRACT(c.data, '$.any_field') LIKE :graphql_filter_builder_parameter2",
            (string)$filter
        );
    }
    ...
    public function testBuildingComplexNestedFilter()
    {
        ...
        $this->assertInstanceOf(Andx::class, $filter);
        $this->assertEquals(
            "c.id = :graphql_filter_builder_parameter1 AND (c.locale LIKE :graphql_filter_builder_parameter2 OR c.locale LIKE :graphql_filter_builder_parameter3 OR (c.locale LIKE :graphql_filter_builder_parameter4 AND c.locale LIKE :graphql_filter_builder_parameter5))",
            (string)$filter
        );
    }
}
