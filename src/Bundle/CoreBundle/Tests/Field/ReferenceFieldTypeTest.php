...
class ReferenceFieldTypeTest extends FieldTypeTestCase
{
    public function testContentTypeFieldTypeWithEmptySettings() {
        ...
        $errors = static::$container->get('validator')->validate($ctField);
        $this->assertCount(1, $errors);
        $this->assertEquals('reference_type_required', $errors->get(0)->getMessageTemplate());
        $this->assertContains('settings', $errors->get(0)->getPropertyPath());
    }
    ...
    public function testContentTypeFieldTypeWithValidUserReferenceSettings() {
    ...
    public function testFormOptionGeneration() {
        ...
        $this->assertEquals('content', $options['attr']['fieldable-type']);
    }

    public function testFormOptionGenerationForDomainMember() {
        ...
        $this->assertEquals('member', $options['attr']['fieldable-type']);
    }
    ...
    public function testDomainMemberTypeFieldContentLabelFallback() {
        ...
        /**
         * @var DomainMemberType $editorType
         */
        $editorType = $ctField->getContentType()->getDomain()->getDomainMemberTypes()->get('editor');

        // No content_label fallback and no content_label set.

        $editorType->setDomainMemberLabel('');
        $ctField->setSettings(new FieldableFieldSettings([
            'domain' => 'foo',
            'domain_member_type' => 'editor',
        ]));
        ...
        $options = $fieldType->getFormOptions($ctField);
        $this->assertEquals((string)$editorType . ' #{id}', $options['attr']['content-label']);

        // No content_label empty and fallback present
        $editorType->setDomainMemberLabel('foo');
        $ctField->setSettings(new FieldableFieldSettings([
            'domain' => 'foo',
            'domain_member_type' => 'editor',
        ]));
    ...
    public function testReferenceToMemberOfAnotherDomain() {
    ...
    /**
     * @expectedException \UniteCMS\CoreBundle\Exception\InvalidFieldConfigurationException
     * @expectedExceptionMessage A reference field was configured with domain member type "baa" on domain "domain1". However "baa" does not exist.
     */
    public function testDomainMemberTypeNotFoundException()
    {
        $ctField = $this->createContentTypeField('reference');
        $ctField->getContentType()->getDomain()->setIdentifier('domain1');
        $ctField->setSettings(
     ...
     * @expectedExceptionMessage A reference field was configured with domain "domain1". However you are not allowed to access it.
     */
    public function testDomainNoAccessWithDomainMemberType()
    {
        $ctField = $this->createContentTypeField('reference');
        $ctField->getContentType()->getDomain()->setIdentifier('domain1');
        $ctField->setSettings(
    ...
    /**
     * @expectedException \UniteCMS\CoreBundle\Exception\ContentTypeAccessDeniedException
     * @expectedExceptionMessage You are not allowed to list members of domain member type "editor" on domain "domain1".
     */
    public function testDomainMemberTypeNoAccess()
    {
        $ctField = $this->createContentTypeField('reference');
        $ctField->getContentType()->getDomain()->setIdentifier('domain1');
        $ctField->setSettings(
