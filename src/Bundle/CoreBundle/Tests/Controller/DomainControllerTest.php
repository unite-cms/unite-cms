...
class DomainControllerTest extends DatabaseAwareTestCase
{
    ...
    public function testCRUDActionsAsAdmin() {
        ...
        $crawler = $this->client->request($form->getMethod(), $form->getUri(), $values, $form->getPhpFiles());
        $this->assertFalse($this->client->getResponse()->isRedirect());
        $this->assertCount(1, $crawler->filter('.uk-alert-danger p:contains("title: '.$this->container->get('translator')->trans('not_blank', [], 'validators').'")'));
        $this->assertCount(1, $crawler->filter('.uk-alert-danger p:contains("identifier: '.$this->container->get('translator')->trans('not_blank', [], 'validators').'")'));
