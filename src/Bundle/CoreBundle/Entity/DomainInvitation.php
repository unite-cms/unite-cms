...
class DomainInvitation
{
     ...
     * @Assert\Valid()
     * @Assert\NotBlank(message="validation.not_blank")
     * @ORM\ManyToOne(targetEntity="UnitedCMS\CoreBundle\Entity\Domain", inversedBy="invites")
     */
    private $domain;
    ...
    public function emailNotAlreadyTaken(ExecutionContextInterface $context)
    {
        if ($this->getDomain() && $this->getDomain()->getOrganization()) {
            foreach ($this->getDomain()->getOrganization()->getUsers() as $organizationMember) {
                if ($organizationMember->getUser()
