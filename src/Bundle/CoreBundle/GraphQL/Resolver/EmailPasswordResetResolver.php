...
class EmailPasswordResetResolver implements FieldResolverInterface
{
    ...
    public function __construct(DomainManager $domainManager, LoggerInterface $uniteCMSDomainLogger, ValidatorInterface $validator, FieldTypeManager $fieldTypeManager, JWTEncoderInterface $JWTEncoder)
    {
    ...
    public function resolve($value, $args, $context, ResolveInfo $info)
    {
        switch ($info->fieldName) {
            case 'emailPasswordResetRequest': return $this->resolveRequest($value, $args, $context, $info);
            case 'emailPasswordResetConfirm': return $this->resolveConfirm($value, $args, $context, $info);
            default: return null;
        }
    }
    ...
    protected function getDirectiveConfig(string $type) : ?array {
        ...
        if(!$emailField) {
            $this->uniteCMSDomainLogger->warning(sprintf('Unknown emailField "%s" for @emailPasswordReset of user type "%s"', $resetDirective['emailField'], $type));
            return null;
        }

        if(!$passwordField) {
            $this->uniteCMSDomainLogger->warning(sprintf('Unknown passwordField "%s" for @emailPasswordReset of user type "%s"', $resetDirective['passwordField'], $type));
            return null;
        }
     ...
     * @param string $email
     */
    protected function sendPasswordRequestEmail(UserInterface $user, string $email) : void {
        // TODO
    }
     ...
     * @return bool
     */
    protected function resolveRequest($value, $args, $context, ResolveInfo $info) : bool {
        ...
        $this->sendPasswordRequestEmail($user, $email);
     ...
     * @return bool
     */
    protected function resolveConfirm($value, $args, $context, ResolveInfo $info) : bool {
        ...
        $domain->getUserManager()->update($domain, $user, [
            $config['passwordField'] => $passwordField->normalizePassword($user, $args['password']),
        ]);
